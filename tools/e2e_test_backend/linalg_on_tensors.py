...
class LinalgOnTensorsTcpBackend(TcpBackend):
    ...
    def compile(self, imported_module: Module):
        ...
        Args:
          imported_module: The MLIR module consisting of funcs in the TCP dialect.
        Returns:
        ...
        run_pipeline_with_repro_report(
            imported_module,
            f"builtin.module(torch-backend-to-tcp-backend-pipeline)",
            "Lowering Torch backend contract to TCP backend contract")

        run_pipeline_with_repro_report(
            imported_module,
            f"builtin.module(func.func({TCP_TO_LINALG_FUNC_PIPELINE}))",
            "Lowering TCP backend contract to Linalg-on-Tensors backend contract")
